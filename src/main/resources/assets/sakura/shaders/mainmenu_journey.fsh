...
#define iChannel0 sampler2D(0)
#define iChannel1 sampler2D(0)

vec4 noiseTexture(vec2 uv) {
    vec2 p = floor(uv * 0.5);
    vec2 f = fract(uv * 0.5);
    ...
    float c = fract(sin(dot(p + vec2(0.0, 1.0), vec2(12.9898, 78.233))) * 43758.5453);
    float d = fract(sin(dot(p + vec2(1.0, 1.0), vec2(12.9898, 78.233))) * 43758.5453);
    
    float n = mix(mix(a, b, f.x), mix(c, d, f.x), f.y);

    float v = mix(0.55, n, 0.1);
    
    return vec4(v, v, v, 1.0);
}

#define textureLod(tex, uv, lod) noiseTexture(uv)
#define texelFetch(tex, p, lod) noiseTexture(vec2(p) / 256.0)
