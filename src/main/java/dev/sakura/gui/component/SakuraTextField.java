...
public class SakuraTextField extends TextFieldWidget {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        NanoVGRenderer.INSTANCE.draw(vg -> {
            ...
            // Border
            if (isFocused()) {
                NanoVG.nvgStrokeColor(vg, SakuraTheme.color(SakuraTheme.BUTTON_BORDER)); // Active border
            } else {
                NanoVG.nvgStrokeColor(vg, SakuraTheme.color(SakuraTheme.BUTTON_BORDER, 0.5f)); // Inactive border
            }
